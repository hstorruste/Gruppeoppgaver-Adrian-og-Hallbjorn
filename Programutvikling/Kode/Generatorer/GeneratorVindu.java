 ...
 * systemet. Det gjøres på følgende måte: Medisiner må man legge inn manuelt i
 * eget register på forhånd. Man kan opprette tilfeldige leger og tilfeldige
 * pasienter, så mange man måtte ønske. (Legenes passord settes til
 * standardpassord: '1234'). Deretter kan man generere et gitt antall resepter
 * for hver pasient, som legges i pasientens reseptregister. Dette gjøres på
 * følgende måte:
 *
 ...
 * dags dato.Generatoren sørger for at pasienten finnes i legens eget
 * pasientregister.
 *
 * Gerneratorprogrammet består av følgene delgeneratorer: Navngenerator
 * PasientGenerator LegeGenerator Reseptgenerator
 *
 * Siste versjon: 13-05-2014 Laget av Hallbjørn Storruste s165519
 *
 * @author Hallbjørn
 */
...
//Konstruktøren tar imot medisinregister, legeregister og pasientregister
    ...
    private void genererResepter(int antall) {
        ...
        for (Pasient runner : pasientListe) {
            int antallLeger = generator.nextInt(MAXLEGER) + 1;
    ...
    private void genererLeger(int antall) {
        ...
        Lege[] legeListe = legeregister.getAlleLeger();
        Random generator = new Random();
        //Setter bevilling B til false på 1 av 50 leger
        ...
        //Setter bevilling A til false på 1 av 20 leger
    ...
    private void resultater() {
        ...
        int totaltMedisiner = medisinSet.size();
        ...
        utskrift += "\nTotalt antall medisiner: " + totaltMedisiner;
        if (totaltLeger > 0) {
        ...
        if (totaltMedisiner > 0) {
            utskrift += "\nGjennomsnittlig antall resepter per medisin: ";
            utskrift += (totaltResepter / totaltMedisiner);
        }
